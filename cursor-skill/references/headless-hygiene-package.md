...
- Desktop-egress is a CDN, not a vault (update computer first; laptop closed stops routing not cloud work). Ownership-pass is access grant + hop. Local-computer-exec is owner-gated (Ask every time). Videos 48h+30d / Eyes Off still retain; None-retention project ≠ lane; CSAM scan overrides ZDR; image gen Eyes Off no. No Legacy Privacy Mode. Do not add allowed-tools write grants or disallowed-tools (one-turn grant/restriction-clear is not a vault). Skill-scan is not a vault. Grok Bot computer is US-hosted (not on-prem). Sign-out ≠ wipe synced skills. Dist-path extra keys error on claude.ai/API. Invention screenshots are still CSAM-scanned.
