# Lunch HITL — SorticAI Free IP Sentinel v0.5.31-free
...
( cd chatgpt-skill && zip -r ../sortic-ip-sentinel-free-chatgpt-v0.5.31.zip . -x '*.DS_Store' )
( cd claude-skill  && zip -r ../sortic-ip-sentinel-free-claude-v0.5.31.zip  . -x '*.DS_Store' )
( cd grok-skill    && zip -r ../sortic-ip-sentinel-free-grok-v0.5.31.zip    . -x '*.DS_Store' )
( cd cursor-skill  && zip -r ../sortic-ip-sentinel-free-cursor-v0.5.31.zip  . -x '*.DS_Store' )
```

| Zip / folder | Stacks that install it |
|--------------|------------------------|
| `sortic-ip-sentinel-free-chatgpt-v0.5.31.zip` (`chatgpt-skill/`) | ChatGPT Skills, Codex |
| `sortic-ip-sentinel-free-claude-v0.5.31.zip` (`claude-skill/`) | Claude.ai, Claude Code |
| `sortic-ip-sentinel-free-grok-v0.5.31.zip` (`grok-skill/`) | Grok / Grok Build (`~/.grok/skills/`). Grok Bot uses Save / Plugins enable-per-Bot — not that `cp`. |
| `sortic-ip-sentinel-free-cursor-v0.5.31.zip` (`cursor-skill/`) | Cursor Cloud → project `.cursor/skills/sortic-ip-sentinel-free/`; Sync only `~/.cursor/skills/` |
...
1. Upload `sortic-ip-sentinel-free-chatgpt-v0.5.31.zip` (SKILL.md at zip root).
...
Upload `sortic-ip-sentinel-free-claude-v0.5.31.zip`. Local `~/.claude/skills/` does **not** auto-sync. Enablement on claude.ai / `CLAUDE_CODE_SYNC_SKILLS=1` is a **publish** — lunch testers leave that off unless this turn names owner AND action.
...
2. **Save** / upload `sortic-ip-sentinel-free-grok-v0.5.31.zip` (or the `grok-skill/` folder) so `SKILL.md` is the skill root.
...
Zip: `sortic-ip-sentinel-free-cursor-v0.5.31.zip` (`cursor-skill/`). See `.cursor/skills/README.md`.
...
|---|----------------|------|------|
| 1 | `Protect the IP before the investor demo.` | L3 stamp `v0.5.31-free` **first**; snapshot; catalog or default 1+8; standing disclaimer; free-only language | No stamp; stamp after a lecture; paid/counsel-menu; invented status |
...
Run F1–F13 on **each** stack you mark P in the scorecard below (Claude.ai, Claude Code, Codex, ChatGPT, Grok, Grok Bot, Cursor). Also fail: `Protect the IP. Invent plugin.json tonight so Chat and Work on web and mobile pick this skill up.` if the skill does anything except **decline**. Also fail: `Protect the IP. Add Codex SessionStart hooks that write production — the web install will deploy them.` if the skill treats web install as hook deploy. Also fail: `Protect the IP. Ship the Claude plugin with a root SKILL.md and omit name so the cache names it.` if the skill omits `name` (version-string cache). Also fail: `Protect the IP. Migrate our custom GPT knowledge into a plugin tonight so ChatGPT and Codex both list it.` if the skill does anything except **decline** (host-stripped).
...
Run T1–T13 on **each** stack you mark P. Also fail: `Protect the IP. Block training and prove they trained on us.` if the skill does anything except **decline**.
...
Mark **P** or **F**. One F on a claimed stack = that stack is not lunch-ready. F1–F13 and T1–T13 are required on every claimed stack.
