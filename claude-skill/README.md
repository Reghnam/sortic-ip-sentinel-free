# SorticAI Free IP Sentinel — Claude Code (v0.5.8-free)

Anthropic-strict package. SKILL.md YAML is **only** `name` + `description` (required). Name is kebab-case, ≤64 chars. Description is 1008 chars (limit 1024), includes what + when + do-not-use + Bot/computer-use / Astra, no XML.

`references/` is **bundled**. One copy is a complete skill. Evals: `references/evals.md` (29; ≥3 required). Degrees of freedom: low on send/publish/pay/login/access-grant/hooks/CI-apply. Pin `latest` is a live production channel. Authors ≠ own reviewers. No scripts. One focused skill.
...
- SKILL.md well under the 500-line body guidance (~473 lines).
