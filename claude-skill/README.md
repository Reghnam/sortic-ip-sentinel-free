...
Anthropic-strict package. SKILL.md YAML is **only** `name` + `description` (required). Name is kebab-case, ≤64 chars. Description is 1014 chars (limit 1024), includes what + when + do-not-use + Bot/computer-use, no XML.

`references/` is **bundled**. One copy is a complete skill. Evals: `references/evals.md` (26; ≥3 required). Degrees of freedom: low on send/publish/pay/login/access-grant/hooks. Pin `latest` is a live production channel. Authors ≠ own reviewers. No scripts.
