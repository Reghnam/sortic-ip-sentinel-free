...
Free portable IP-sensitive moment detector + hygiene sentinel (**v0.5.1-free**).
...
Works on **Codex**, **ChatGPT Skills**, **Claude Code**, **Grok / Grok Build**, Cursor and any [agentskills.io](https://agentskills.io) runtime.

See [CHANGELOG.md](CHANGELOG.md) for what landed in v0.5.1.
...
| **Anthropic Claude Code** | [`claude-skill/`](claude-skill/) | `cp -r claude-skill ~/.claude/skills/sortic-ip-sentinel-free` then copy `references/` into it |
...
Headless one-shot (Codex / Claude / grok `-p`): add "Output numbered options and hygiene package JSON."
...
| Description | Front-load triggers (list may be truncated to 8k / 2% context) | What + when + do-not-use | Same description |
| Invocation | ChatGPT `@` · Codex `$` / `/skills` · implicit | implicit + `/skill` | implicit + `.grok/skills` |
| Headless | Numbered 1–8 + JSON | Numbered 1–8 + JSON | Numbered 1–8 **default** |
