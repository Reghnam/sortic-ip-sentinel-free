...
- Description **1024 chars** (under Codex 8k / 2% truncation and Anthropic 1024). Front-loads CSAM, none-retention, US-host, skill-scan. HiTL/UAT stay in the body.
- **Invention screenshots/diagrams are still CSAM-scanned.** CSAM guidance 2026-09-15 plus your-data: image/file inputs are scanned on submit; a classifier hit is retained for manual review even on ZDR/MAM/Eyes Off. A schematic is not exempt. Computer-use screenshots are image inputs.
- `$skill-creator`: keep descriptions short while making WHEN clear. Progressive disclosure — do not read every reference before every change. `context: fork` is a hop.
- `agents/openai.yaml` `default_prompt` names disallowed-tools one-turn, skill-scan ≠ vault, US-host, sign-out ≠ wipe, dist-path extra keys, invention-screenshot CSAM, `context: fork`.
- New evals 80–85: disallowed-tools restriction-clear, skill-scan ≠ vault, US-host, sign-out ≠ wipe, dist-path extra keys, invention-screenshot CSAM.

### Anthropic (second)

- YAML still **only** `name` + `description` on `claude-skill/`. Name kebab-case ≤64. Description **1024 chars**, no XML. Body under 500 (465 body lines).
- **`disallowed-tools` restriction-clear is one-turn.** Same clock as `allowed-tools` grant-clear. Cannot remove `EndConversation` if other tools remain. This skill does not add `disallowed-tools` or `allowed-tools`. One-turn restriction-clear is not a vault.
- **Dist-path extra keys error on claude.ai / Skills API / `package_skill.py`.** Claude Code accepts all frontmatter. That is why `claude-skill/` is name+description only.
- **Skill security scanning is not a vault.** claude.ai/Cowork only. Misses Skills API, already-uploaded skills, CMEK/ZDR/HIPAA orgs. ZDR orgs do not get scanning. Inspect-before-attach still applies.
- **Sign-out ≠ wipe (2.1.272).** claude.ai-synced skills stay on disk until `cleanupPeriodDays`, then trash. Local `~/.claude/skills/` still does not auto-sync. Skills still not covered by ZDR. Evals now **85** (≥3 required). Eight options, not nine. References one level deep. No scripts.

### Grok Build (third)

- **Grok Bot computers run in the United States today.** Not on-prem, not BYO image, not inside your perimeter. Holdbacks on that computer are a US geography fact, not an EU vault. Recreate / image update can drop in-computer sign-in sessions; terminate-computer still keeps the durable disk.
- Auto Review still does not review memory writes / most settings changes. Rules remain desktop-local. Cloud ≠ egress ≠ local-exec unchanged from 0.5.16.
- Galaxy / role livestream calendar add and Claude magic-link stay L0 attend. Streaming internals remains L2/L3.
- No Grok chat-history connector. Live `isEnabled` is evidence. Weekly backup ≠ publish. GitHub auto-push of this public repo: decline unless this turn names owner AND action. (This Grok Build turn *is* the named owner push of hygiene-only files.)

### Shared

- JSON `sorticai.hygiene_package.v1` edition `0.5.17-free`. `agent_exposure.channel` may be `disallowed-tools | skill-scan | us-host | sign-out-wipe | dist-path | csam-shot` (plus prior values).
- Classification: L2-31 disallowed-tools, L2-32 skill-scan, L2-33 US-host; L3-DISALLOW, L3-SKILLSCAN, L3-USHOST, L3-SIGNOUT, L3-DISTPATH, L3-CSAMSHOT; L0-25 Claude magic-link.
- Output-language rules 57–60: disallowed-tools restriction-clear; skill-scan ≠ vault; US-host; dist-path / sign-out / invention-screenshot CSAM.
- Stamp: `v0.5.17-free` / patched 2026-09-15. First release date remains 2026-08-17.
- Still free-only. Still not legal advice. Still silent on meta/privacy/corpus.

## [0.5.16-free] — 2026-09-15

Patched from: operator mailbox 14–15 Sep 2026 (Grok Bot Galaxy starts-tomorrow + role livestream calendar adds — Engineers / Founders / PMs / Marketing — stay **L0 attend**; Claude.ai magic-link stays **L0**; ChatGPT Business payment-failed notices stay **L0 billing**; weekly backup ≠ publish; leftover drafts — accounting reply, UAT leftover, partner pre-read — **unsent**; infra/IAM / Headless-callable partner mail stays out of this public repo — **no client/product/infra names, no valuation figures**); OpenAI recrawl 2026-09-15 (developers.openai.com/api/docs/guides/your-data + tools-skills: **CSAM scan of image/file inputs overrides ZDR/MAM/Eyes Off** — classifier hit retained for manual review; `/v1/videos` still **48h processing + 30d abuse**, currently **blocked for MAM/ZDR**; a project set to retention **None** is required to call videos — **None-retention ≠ protected lane**; gpt-image-2.5-sunburst/flare incl. 2026-09-08 are **ZDR-yes / Eyes Off no**; Safety Retention can pull a model out of ZDR/MAM (notified); three skill form factors — local path / hosted `skill_reference` / Agents `capability_directories` max 32; zip 50 MB / 500 files / 25 MB uncompressed; ZDR-ineligible list unchanged: conversations+items, agents, files, vector_stores, evals, assistants, threads+messages/runs/steps, chatkit, batches, videos, fine_tuning); Anthropic Agent Skills recrawl 2026-09-15 (platform.claude.com + code.claude.com/docs/en/skills: kebab ≤64, description ≤1024, body <500, YAML **name + description only** on `claude-skill/`, listing truncate **description + when_to_use 1536**, `allowed-tools` auto-approves listed tools for **one turn then clears** — one-turn grant-clear is not a vault, do not add write grants; Skills not covered by ZDR; custom claude.ai Skills are per-user and do not auto-sync; Anthropic settings enablement may sync to Cowork — neither is a vault); Grok Bot / Build recrawl 2026-09-15 (docs.x.ai/grok-bot/approvals-security-and-privacy last updated 2 Sep + computer-and-apps: **cloud computer ≠ desktop-egress ≠ local-computer execution** — Settings → General → Agent → Execution on Local Computer, default **Ask every time**, use **Never allowed** unless named; laptop closed continues cloud work, stops egress routing and local-exec; Auto Review rules live on **this desktop** and sync to its Bot computer; **Update Agent Computer** preserves durable state, **Reset** drops unsynced recent work; Network Controls is Enterprise-only; no Legacy Privacy Mode; training opt-out follows the Cursor account; shared Bot computer remains account-wide). **No Grok chat-history connector exists**; Grok-chat insights are from those Bot/Build docs, this Grok Build turn, live automations (`isEnabled` is evidence; this improvement job is enabled), and X ingest. Do not dump client/product facts into the public repo.

### OpenAI (first)

- Description **1019 chars** (under Codex 8k / 2% truncation and Anthropic 1024). Front-loads local-exec, CSAM-override, none-retention videos. HiTL/UAT stay in the body.
- **CSAM scan overrides ZDR/MAM/Eyes Off.** Image/file inputs are scanned on submit; a classifier hit is retained for manual review even on ZDR. gpt-image-2.5-sunburst/flare (incl. 2026-09-08) are ZDR-yes but Eyes Off no. Safety Retention can pull a model out of ZDR/MAM (notified).
- **None-retention videos project ≠ protected lane.** `/v1/videos` stays 48h+30d and blocked for MAM/ZDR. A project set to retention None is required to call it — that is the opposite of a vault. Eyes Off / Safety Retention still retain.
- Three skill form factors restated: local path (no `skill_reference`), hosted `skill_reference` (pin an integer), Agents `capability_directories` (max 32).
- `agents/openai.yaml` `default_prompt` names local-exec ≠ egress ≠ cloud, CSAM override, image Eyes Off no, None-retention ≠ lane, grant-clear one-turn, Galaxy/role L0, Auto Review desktop-local / Update vs Reset.
- New evals 74–79: local-exec, CSAM/image Eyes Off, None-retention videos, grant-clear, Galaxy/role L0, Auto Review desktop-local / Update vs Reset.

### Anthropic (second)

- YAML still **only** `name` + `description` on `claude-skill/`. Name kebab-case ≤64. Description **1019 chars**, no XML. Body under 500 (461 body lines).
- **`allowed-tools` grant is one-turn then clears.** Listed tools auto-approve for the invoking turn; the grant clears on the next message. One-turn grant-clear is not a vault. This skill does not add `allowed-tools`. Extra YAML keys stay off the Claude copy.
- Custom claude.ai Skills are per-user and do not auto-sync. Anthropic settings enablement may sync to Cowork. Neither is a vault. Listing truncate: combined description + `when_to_use` 1536 chars.
- Skills still not covered by ZDR. Evals now **79** (≥3 required). Eight options, not nine. References one level deep. No scripts.

### Grok Build (third)

- **Cloud computer ≠ desktop-egress ≠ local-computer execution.** Local-exec is Settings → General → Agent → Execution on Local Computer (Ask every time default; Never allowed unless named). Laptop closed: cloud work continues; egress routing stops; local-exec needs the desktop. Do not park holdbacks on any of the three. Decline unnamed Always-allowed.
- **Auto Review rules are desktop-local.** They live on this desktop and sync to its Bot computer — another desktop is not the same config. Update Agent Computer preserves durable state; Reset drops unsynced recent work. Network Controls is Enterprise-only; shared static egress; no DLP; no Cursor VPN.
- Galaxy / role livestream calendar add and Claude magic-link stay L0 attend. Streaming internals remains L2/L3. Invite copy that each Bot has its own computer is marketing, not isolation.
- No Legacy Privacy Mode. Training opt-out follows the Cursor account. Shared Bot computer remains account-wide.
...
- JSON `sorticai.hygiene_package.v1` edition `0.5.16-free`. `agent_exposure.channel` may be `local-computer-exec | csam-override | none-retention-project | grant-clear | image-zdr` (plus prior values).
- Classification: L0-24 Galaxy/role livestream calendar add; L2-29 local-exec; L2-30 CSAM/image Eyes Off; L3-LOCAL, L3-CSAM, L3-NONEPROJ, L3-GRANTCLR, L3-IMGZDR.
- Output-language rules 53–56: local-exec ≠ egress ≠ cloud; CSAM override / image Eyes Off no; None-retention videos project ≠ lane; allowed-tools grant-clear is one-turn not a vault.
- Stamp: `v0.5.16-free` / patched 2026-09-15. First release date remains 2026-08-17.
- Still free-only. Still not legal advice. Still silent on meta/privacy/corpus.
...
- Description **1021 chars** (under Codex 8k / 2% truncation and Anthropic 1024). Front-loads protected-lane, API≠chat, Bot-to-Bot, Astra computer-use.
- **API no-train ≠ ChatGPT no-train.** API data is not used to train unless the org opts in (since 2023). Consumer / Business / Enterprise chat is a different lane. Astra workspace-wide computer/browser use is still ChatGPT lane. Model marketing ("most secure") is not a data-control change.
- **ZDR-ineligible endpoints** even when the org has ZDR: conversations, agents, files, vector stores, evals, assistants, threads, chatkit, batches, videos, fine-tuning. Hosted container discarded ≠ 30-day abuse-monitor gone.
- `default_version` / omit-version / `latest` remain live pointers. Pin an integer in production. `capability_directories` max 32.
- Three zip layouts unchanged. Zip 50 MB / 500 files / 25 MB uncompressed. Inspect before attach. No MCP. No lifecycle hooks.
- `agents/openai.yaml` `default_prompt` names API≠chat, Skills ZDR-ineligible (incl. chatkit/batches/videos/fine-tuning), screens≠boundary, Bot-to-Bot, terminate-computer keeps disk, cap 32.
- New evals 56–61: API≠chat, Skills-not-ZDR, Bot screens, Bot-to-Bot DM, protected-lane check, capdir / ZDR-ineligible endpoints. Contents list now includes 50–55 as well.

### Anthropic (second)

- YAML still **only** `name` + `description` on `claude-skill/`. Name kebab-case ≤64. Description **1021 chars**, no XML. Body under 500.
- **Skills are not covered by ZDR.** Definitions *and* execution data are retained. Covered-model 30-day safety retention can apply even on ZDR orgs.
- Custom Skills still do not sync across claude.ai / API / Claude Code. Each surface is a separate publish.
- Gotchas: API≠chat; Skills-not-ZDR; protected-lane check is hygiene not a product; 20-skill session cap; `user-invocable:` only literal `true`; extra YAML keys ignored; `bypassPermissions` is not owner approval; pin `latest`.
- Evals now 61 (≥3 required). Eight options, not nine. References one level deep. No scripts.

### Grok Build (third)

- **Screens ≠ security boundary.** Separate Bots are not a vault. Isolated credentials need a dedicated user.
- **Delete Bot ≠ wipe computer.** Enterprise terminate-computer keeps the durable disk (next session remounts it).
- **Bot-to-Bot DM / group chat is a disclosure hop.** Auto Review does not review memory writes / most settings changes.
- Shared Bot computer remains account-wide (files, sessions, logins). Shared static egress, no dedicated per-customer IPs.
- Public Bot livestream of internals is a demo; attending Galaxy is L0. Voice screen-share sees the browser. Public `x.ai/bot/…` is cloneable config.
- Meeting-notes AI plan downgrade does not stop transcription or Slack auto-share.
...
- JSON `sorticai.hygiene_package.v1` edition `0.5.13-free`. `agent_exposure.channel` may be `protected-lane | skill-zdr | bot-dm | bot-screen | api-vs-chat | capdir` (plus prior values).
- Classification: L2-23 Bot-to-Bot, L2-24 screens; L3-APICHAT, L3-SKILLZDR, L3-BOTDM, L3-SCREEN, L3-LANE, L3-CAPDIR.
- Output-language rules 41–44: API≠chat; ZDR-ineligible (incl. chatkit/batches/videos/fine-tuning); screens ≠ boundary / terminate-computer keeps disk / meet-notes plan-downgrade; protected-lane is hygiene.
- Stamp: `v0.5.13-free` / patched 2026-09-13. First release date remains 2026-08-17.
- Still free-only. Still not legal advice. Still silent on meta/privacy/corpus.

## [0.5.12-free] — 2026-09-13

Patched from: david@ mailbox 12–13 Sep 2026 (de-identified chat insights can train the next AI — PII-stripped / ZDR / Business / Enterprise / "we don't train" is not a vault; opt-out after the chat is not a rewind; a public clip about the industry issue is L0; livestream calendar add is L0 attend; leftover drafts unsent; access grant is owner-gated; weekly backup PARTIAL ≠ publish; workspace billing/payment without protect intent stays L0; live `isEnabled` is evidence — this improvement job is enabled even with a stale nextRun; Sentinel→GitHub auto-push is owner L3 — this Grok Build turn *is* the named owner push of hygiene-only files — **no client/product/infra names, no valuation figures in this repo**); OpenAI recrawl 2026-09-13 (developers.openai.com/plugins/build/skills + **api/docs/guides/tools-skills**: `default_version` is used when `skill_reference.version` is omitted; `latest_version` tracks the newest upload; pin an integer in production; you cannot delete the default version without setting another first; deleting the last remaining version deletes the skill; skill instructions are **user-prompt** input (same priority as other user instructions; this-turn user instructions still beat skill guidelines); inline base64 zip is a publish; curated first-party skills (`openai-spreadsheets`) are a demo surface if they sit in a container that holds the work; inspect before attach; hosted/container skills are discarded when the container expires; zip limits **50 MB / 500 files / 25 MB uncompressed**; three zip layouts stay distinct; do not expose this public repo as an open end-user catalog); Anthropic Agent Skills recrawl 2026-09-13 (no new name/description limits: kebab ≤64, description ≤1024 — this patch **trims description to 1014 chars**; SKILL.md body <500; references one level deep; ≥3 evals; Skills are not a ZDR boundary; **custom Skills do not sync** across claude.ai / API / Claude Code — each install is a separate publish; 20-skill session cap; `user-invocable:` only literal `true`; extra YAML keys ignored; `bypassPermissions` is not owner approval; folder name must match `name`; Windows paths: forward slashes); Grok Bot / Build recrawl 2026-09-13 (`docs.x.ai/grok-bot/overview` last updated 11 Sep: **all of your Bots use the same cloud computer, sharing its files, browser sessions, and app logins** — the computer belongs to the account, not to an individual Bot; treat anything placed on it as available to every Bot you run; between users, isolation is strict; demonstration-to-skill still a publish; X ingest 13 Sep: a marketplace skill that lets a Bot take jobs and get paid onchain is **access grant + send + computer-use**). **No Grok chat-history connector exists**; Grok-chat insights are from those Bot/Build docs, this Grok Build turn, live automations (stale nextRun ≠ didn't run; live `isEnabled` is evidence; weekly backup ≠ publish), and X ingest. Do not dump client/product facts into the public repo.

### OpenAI (first)

- Description **1014 chars** (under Codex 8k / 2% truncation and Anthropic 1024). Front-loads de-identified training, ZDR, `default_version`. States decline for login, DNS, unnamed repo push, partner send, pay, identity, access grant, webhook, voice/phone, CI auto-apply, Sites public publish, counsel-release.
- **`default_version` is a live pointer.** Omitting `skill_reference.version` uses `default_version`. Changing default_version or pinning `latest` is a live-channel switch. Cannot delete the default without setting another first. Production should pin an integer.
- Skill instructions are **user-prompt** input. This-turn user instructions still beat skill guidelines. On L3 decline, name and link SKILL.md.
- Inline base64 zip and curated first-party skills are a demo surface. Hosted/container is discarded on expiry. Inspect before attach. Do not expose this public repo as an open end-user catalog.
- Three zip layouts unchanged. Zip 50 MB / 500 files / 25 MB uncompressed. No secrets. No MCP. No lifecycle hooks.
- `agents/openai.yaml` `default_prompt` names de-identified/ZDR, `default_version` / omit-version, shared Bot sessions, marketplace job-taking, cross-surface Claude. Implicit invocation stays on.
- New evals 50–55: de-identified/ZDR, public clip L0, default_version live pointer, shared Bot session, marketplace job-taking, cross-surface Claude.

### Anthropic (second)

- YAML still **only** `name` + `description` on `claude-skill/`. Name kebab-case ≤64. Description **1014 chars**, no XML. Body under 500.
- **Custom Skills do not sync** across claude.ai / API / Claude Code. Each surface is a separate publish.
- Skills are not a ZDR boundary. PII-stripped / ZDR-claimed training still absorbs invention-grade content.
- Gotchas: de-identified/ZDR; `default_version`; shared Bot sessions; marketplace job-taking; cross-surface install; 20-skill session cap; `user-invocable:` only literal `true`; extra YAML keys ignored; `bypassPermissions` is not owner approval; authors ≠ own reviewers; pin `latest`.
- Evals now 55 (≥3 required). Eight options, not nine. References one level deep. No scripts.

### Grok Build (third)

- **Shared Bot computer is account-wide.** Files, browser sessions, and app logins are available to every Bot you run. Isolation between users is strict; isolation between your Bots is not. Do not park holdbacks there.
- **Marketplace job-taking skill is access grant + send + computer-use.** Decline unnamed.
- De-identified / ZDR-claimed frontier chat of invention-grade content is a disclosure. Opt-out is not a rewind. A public clip about the industry issue is L0. Livestream calendar add is L0 attend.
- Live `isEnabled` is evidence. This job fires when live metadata says enabled. Stale nextRun ≠ didn't run. Weekly backup ≠ publish.
...
- JSON `sorticai.hygiene_package.v1` edition `0.5.12-free`. `agent_exposure.channel` may be `deid-train | default-version | shared-bot-session | job-skill | cross-surface` (plus prior values).
- Classification: L0-20 public clip, L0-21 livestream calendar add; L2-21 shared Bot session, L2-22 de-id/ZDR paste; L3-DEID, L3-DEFVER, L3-JOBSKILL, L3-XSURFACE.
- Output-language rules 37–40: de-identified/ZDR; `default_version` / omit-version; shared Bot computer; marketplace job-taking / cross-surface Claude.
- Stamp: `v0.5.12-free` / patched 2026-09-13. First release date remains 2026-08-17.
- Still free-only. Still not legal advice. Still silent on meta/privacy/corpus.
