...
4. **No invented status.** Unknown stays unknown. Do not invent, replace, or reroll a result. Do not claim sent / filed / approved / didn't-run without evidence in this turn. Stale nextRun ≠ didn't run.
5. **Approval gate.** Do not email, post, auto-publish, push, pay, publish identity/live-copy, or send leftover drafts / partner pre-reads unless this turn names the recipient **and** the action. Empty signature replies stay unsent. Claimed send needs message-id + recipients.
...
5c. **Owner-gated.** Send / publish / pay / identity / live title-copy stay declined even at IP L3 unless named this turn.
...
Set `"output_register": "procedural_builder_worksheet"`, `"not_for_third_party": true`, `"approval_required": true`, `"owner_gated": true` on `sorticai.hygiene_package.v1`. Repeat the banner inside `disclaimer`. Emit `agent_exposure` (empty array is fine) and `evidence_or_blocked`. Computer-use / livestream / HiTL / board is a demo channel — see `agent-exposure-log.md`.
...
- [ ] About to email, post, pay, or auto-publish? Recipient **and** action named this turn? Not a leftover draft or partner pre-read?
...
63. **De-ID ≠ method safe. Exhaust ≠ proved training. ZDR ≠ vault.** The approach is the IP. A podcast anecdote is not audited proof of training (All-In X https://x.com/theallinpod/status/2098606000890687813 / YT https://www.youtube.com/watch?v=cvxjqbfLVk0 — public URL located). Sacks's unidentifiable-data question is not a finding. Multi-turn corrections and tool traces are intelligence exhaust (Satya Reverse Information Paradox — public URL located, not verified). ZDR is contractual best-efforts. Never write "they trained on you". This skill **cannot prove or prevent training**.
