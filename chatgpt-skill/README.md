# SorticAI Free IP Sentinel — ChatGPT / Codex (v0.5.8-free)
...
Invoke: `$sortic-ip-sentinel-free` or `/skills`. Implicit match uses the description (front-loaded triggers, 1008 chars). Headless `-p`: unnamed L3 **default-delivers 1 (show/hold) + 8 (JSON)**; or add "Output numbered options and hygiene package JSON."
...
- Computer-use / Bot email-push-post / callable hop is a demo channel (catalog item 5 + `agent_exposure` JSON). Never email leftover drafts, partner pre-reads, or fit-notes unless this turn names recipient AND action. Backup is not publish. `--yolo` is not owner approval. Voice/phone provision is declined. Skill zip is scanned — no secrets; scan again after change before plugin submit. MCP Scan Tools is a snapshot (not a live fetch). Plugin-bundled hooks stay skipped until trusted; this skill has none. Reviewer hop / mailbox / Grok-share / access grant / drafts≠send / voice screen-share apply. Done requires path+link.
