...
- This is the ChatGPT Skills / Codex variant (minimal frontmatter + agents/openai.yaml).
- Canonical rich frontmatter lives at repo root SKILL.md.
- OpenAI 2026-09-06 plugins/skills: description is the match rule (front-load triggers; list may truncate to 8k / 2% context). State what the model must not infer; when to stop or decline. `allow_implicit_invocation: true`. No MCP dependencies (instructions over scripts; a dependency does not replace workflow). Zip with SKILL.md at zip root for ChatGPT Skills. Optional plugin wrap later — skill folder stays the install unit.
