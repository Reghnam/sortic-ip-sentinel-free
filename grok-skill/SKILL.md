...
when-to-use: >
  ...
  pilot showcase, fundraising deck, HiTL, UAT, livestream, board, partner pre-read, callable agent,
  reviewer hop, mailbox connector, Grok share, App Builder preview, Vercel deploy, claude.ai sync, voice/phone provision, Grok Bot email/push/post/auto-publish of the work.
  Silent on privacy/config/meta/skill-work, workspace renewal, weekly backup, model pick, and the US IP corpus. Free hygiene only.
user-invocable: true
disable-model-invocation: false
argument-hint: IP sensitive moment | show/hold | hygiene JSON
metadata:
  short-description: "SorticAI Free IP Sentinel v0.5.14-free — Astra complete + claude.ai sync + App Builder≠Vercel; yolo≠approval"
  version: "0.5.14-free"
  release: "2026-08-17"
  patched: "2026-09-14"
...
- Grok Build: the user has chat + live preview only — never ask them to run shell commands.
- Grok Bot: persistent teammate with its own computer. Treat email/push/browser/social as demo channels. Never invent status. Never email third parties unless this turn names them.
- `argument-hint` is for slash-command autocomplete. `disable-model-invocation: false` so implicit L2/L3 still works.
