...
These rules come from counsel review of adjacent SorticAI writing (2 Sep 2026), Task Extractor / Grok Bot operating notes (do not invent status; do not email third parties unless named), plus Agent Skills authoring practice (Anthropic: no voodoo constants; OpenAI: plain I/O). They are **writing rules**, not legal opinions.
...
4. **No invented status.** Unknown stays unknown. Same rule Grok Bot ops use: do not claim sent / filed / approved without evidence in this turn.
5. **No third-party send.** Do not email, post, or push hygiene or IP content to third parties unless this turn names the recipient.
...
Set `"output_register": "procedural_builder_worksheet"` and `"not_for_third_party": true` on `sorticai.hygiene_package.v1`. Repeat the banner inside `disclaimer`. Emit `agent_exposure` (empty array is fine). Computer-use is a demo channel — see `agent-exposure-log.md`.
...
- [ ] About to email or post? Recipient named this turn?
