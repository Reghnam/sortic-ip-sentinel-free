...
description: >
  Free IP-sensitive moment detector and hygiene sentinel (v0.5-free portable edition).
  Activate on explicit protect language (protect IP, patent, trade secret, NDA, prior art,
  inventorship, IP analysis, IP checkpoint, "IP sensitive moment") or strong exposure signals
  (investor demo, pilot showcase, fundraising deck). Deliver only free procedural hygiene:
  show/hold maps, staged disclosure, demo hygiene playbook, AI contribution logs, provenance
  audits, provisional checklists, lite prior art pointers, disclaimers. No paid paths, no
  prices, no firm names, no protectability rulings. Silent on privacy/config/meta/skill-work.
  Progressive disclosure: load references on demand. Not legal advice. No guarantees.
  Matches agentskills.io SKILL.md open standard for Claude Code, Grok, Cursor and compatible agents.
when-to-use: >
  ...
  Do not activate for generic privacy, telemetry, security config, debugging, or meta discussion
  of this skill. Primary task always wins. Free hygiene only.
user-invocable: true
disable-model-invocation: false
metadata:
  short-description: "SorticAI Free IP Sentinel v0.5-free — detect sensitive moments + deliver powerful free hygiene only"
  version: "0.5-free"
  release: "2026-08-17"
  author: "SorticAI (portable free tier)"
  category: "ip-hygiene"
  tags: ["ip", "patent", "trade-secret", "hygiene", "demo-safety", "inventorship", "free"]
license: "See LICENSE in distribution repo (proprietary for now; free use with attribution and disclaimers)"
compatibility: "Claude Code (~/.claude/skills/), Grok (~/.grok/skills/ or .grok/skills/), Cursor, compatible MCP/agentskills runtimes. No external services required for core."
---

# SorticAI IP Sentinel — Free Edition (v0.5-free)
...
- **L3 Full sentinel (explicit IP-sensitive moment):** Protect language + analysis or strong nouns (protect the IP / trade secret / NDA / prior art / inventorship / patent / "IP sensitive moment" / IP analysis / how to protect this / run analysis and check) OR clear combo of exposure + protect intent. Or direct invocation.
...
4. Free hygiene offer list (interactive if supported, or plain bullets).
5. Prominent disclaimer.
6. Deliver chosen items with templates + disclaimers.
...
|  v0.5-free (portable)  ·  sorticai.com  ·  released 2026-08-17               |
...
Use the list below. Deliver practical artifacts (maps, checklists, text, tables). Always end with sources note and disclaimer. See references/ for full templates when present.

- **Show / hold map:** Safe to show live · keep private · wait for NDA + provisional before deeper detail. Use staged disclosure ladder.
- **Staged disclosure ladder + patent presentation template:** High-level teaser (problem + outcome) → block architecture + investor-level pipeline (no exact recipes/parameters/full source) → deeper only under NDA. Last slide always disclaimer.
- **Investor / Partner Demo Hygiene Playbook:** Pre-demo checklist (7-10 days), demo-mode creation, access controls, logging what was shown, ground-rules script, run-of-show (story → rules → demo → business → hold on disclaimer), Q&A armor.
- **AI / Agentic Invention Contribution Log:** Table — Date | Human contributor | Specific contribution (problem framing, prompt design/selection/refinement, recognition of result, integration/validation) | Evidence pointer (redacted chat/note). Supports human conception hygiene.
- **Provisional Readiness Checklist:** Enablement elements (technical problem, specific implementation + effect/metrics, diagrams/pseudocode, embodiments). Timing: file before significant exposure. Jurisdiction note (US grace vs most places absolute novelty). Follow-on provisionals.
- **Code / Output Provenance + Holdback Audit:** Scan artifacts for AI-generated sections. Log what was shown to LLM. Mask/redact for demos. "What not to put in decks/repos/recordings."
- **Trade Secret ID Matrix + Secrecy Measures:** Asset | Why candidate (hard to reverse-engineer?) | Current steps (marking, access, NDAs, no public AI for core) | Gaps. List reasonable measures.
- **Lite Prior Art / FTO High-Level Signals:** Quick public search pointers (USPTO PatFT/AppFT, Google Patents, Espacenet, WIPO PATENTSCOPE, Lens.org). Red flags for software/AI (abstract claims, "apply ML to X", common techniques). Explicit: "This is not a search and not clearance." Escalate to qualified counsel for real work.
- **Session ground rules text, pre-meeting pack, slides help (disclaimer last slide), standalone disclaimer, situation summary, Q&A armor, hold-back checklist, run-of-show.**
...
Keep this SKILL.md lean. When user selects a deliverable that needs detail:
- Load and follow `references/investor-demo-hygiene-playbook.md` (staged ladder, presentation template, checklist, run-of-show).
- Load `references/free-tier-outcomes.md` or equivalent catalog for full offer list.
- Use or adapt templates for contribution log (table), provisional checklist, TS matrix, etc.
- For any export/summary: include the standing disclaimer + sources note.

Instruct the agent: "Read the referenced file now and apply its structure exactly. Do not invent new legal-sounding claims."
...
- Every deliverable ends with disclaimer + "Sources referenced (high-level summaries): EPO..., USPTO..., WIPO...".
- Version in header: v0.5-free.
- Headless friendly: JSON or structured markdown + disclaimer when user requests export/summary.
...
- This is the portable free tier. Extracted from full sortic-ip-checkpoint (v1.6) tier logic + free outcomes.
- Update references/ alongside. Keep disclaimers, plain labels, hygiene scope.
- Test triggers: explicit protect + "IP sensitive moment" → L3; meta editing this → L0.
- Distribution: GitHub folder + ZIP + extracted prompt copy-paste. See v05-suggestions.md and v05-research-summary.md.
- Current version: v0.5-free (2026-08-17). No paid elements.
