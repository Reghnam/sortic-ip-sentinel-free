...
description: >
  ...
  will show, email, push, post, auto-publish, send leftover drafts, pin default_version, or treat a backup as publish.
...
metadata:
  short-description: "SorticAI Free IP Sentinel v0.5.12-free — de-identified training + default_version + shared Bot sessions; free hygiene only"
  version: "0.5.12-free"
  release: "2026-08-17"
  patched: "2026-09-13"
...
# SorticAI IP Sentinel — Free Edition (v0.5.12-free)
...
|  v0.5.12-free (portable)  ·  sorticai.com  ·  patched 2026-09-13              |
...
- **De-identified training is still a disclosure.** Putting invention-grade content into a frontier chat is L2 without protect language, L3 with it — even if ZDR / Business / Enterprise / "we don't train" / PII-stripped training is claimed. Opt-out after the chat is not a rewind. A public clip about the industry issue is L0.
- **`default_version` is a live pointer.** Omitting `skill_reference.version` uses `default_version`. Changing default_version (or pinning `latest`) is a live-channel switch. You cannot delete the default version without setting another first. Skill instructions are user-prompt input (same priority as other user instructions; this-turn user instructions still beat skill guidelines). Inline base64 zip and curated first-party skills (`openai-spreadsheets`) are still a demo surface if they sit in a container that holds the work.
- **Shared Bot computer is account-wide.** All of your Bots share files, browser sessions, and app logins. Treat anything on that computer as available to every Bot you run. Isolation between users is strict; isolation between your Bots is not. A marketplace skill that lets a Bot take jobs or get paid is access grant + send + computer-use. Custom Skills do not sync across Anthropic surfaces (claude.ai vs API vs Claude Code) — each install is a separate publish.
...
- **De-identified / ZDR is not a vault.** PII-stripped training still absorbs organizational IP. "We don't train" / Business / Enterprise / ZDR marketing is not a rewind of a chat already sent. Skills are not a ZDR boundary (Anthropic).
...
- **Shared Bot sessions are account-wide.** Files + browser sessions + app logins are available to every Bot you run. Cross-surface Claude install is a separate publish. Marketplace job-taking is access grant + send.
...
- Version in header: v0.5.12-free.
...
- About to re-enable a disabled job, rewrite a live prompt, auto-apply an infra PR, publish `latest` / change `default_version` unnamed, attach this public skill as an open end-user catalog, submit a Claude-plugin archive unnamed, livestream internals, treat a judge-model as counsel, treat ZDR as a vault, park holdbacks on a shared Bot computer, install a marketplace job-taking skill unnamed, or run exploit/PoC → decline. Pin an integer version (not omit-version / default_version / latest). One focused skill. Three zip layouts; do not mix. Counsel-unreviewed client send is a hold. De-identified training is still a disclosure.
...
- Current version: **v0.5.12-free** (patched 2026-09-13; first released 2026-08-17). Description ≤1024. Body stays well under 500 lines.
